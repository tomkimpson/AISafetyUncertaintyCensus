# Dangerous-Capability Eval Corpus (legacy narrative extraction)

> **Legacy file:** this Markdown table preserves the original 98-record research pass and
> is not an analysis input. The canonical structured file is `census_records.csv`, which
> explicitly excludes T7 and A32 from the 96-record primary corpus and codes denominator
> and uncertainty status without text heuristics. Exact paper counts come from that CSV.
...
`evals.csv` holds the ten model-level audit results; shared source record A4 maps to two models.

**Census scope (through 2026-07-03; provenance re-verified 2026-07-10):** the census is a
screened set of **98 candidate records** spanning the Claude 4 / Gemini 2.5 /
...
paper's count macros (`\CensusTotal`, `\CensusScoreN`, `\CensusUncertainty`, …) are
computed by `scripts/make_tables.py` over the **96 eligible records in the canonical CSV** — the post-2025 rows are part of
...
out of any total. The ten model-level audit results (`evals.csv`) map to nine source records.
The key cross-generation finding (each developer supplies at most one of {numeric
...
post-2025 rows as excluded; that split was a construction-time artifact. Do not reintroduce
the "~60" framing or use this legacy table to recompute current counts.*
...
The primary corpus contains 96 eligible records across four framework families (exact counts are machine-generated by
`scripts/make_tables.py` into `census_counts.tex`; do not hand-maintain them here). Of the
96: 73 report the denominator tied to the displayed score, 8 report partial or indirect
sample information, and 15 report none; only 15 report any uncertainty (a CI
...
- **Anthropic Opus 4 bio uplift trial** (reported alongside the precautionary ASL-3
